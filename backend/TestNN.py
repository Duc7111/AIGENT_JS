...
set_module_hyperparameters(key='vectorHolder', hyperparameters={'vector': [0, 0]})
...
set_module_hyperparameters(key='neuralUnit1', hyperparameters={'weights': [1, 1], 'bias': 0, 'activation': lambda x: x if x > 0 else 0})

add_module('neuralUnit2', 'NN', 'NeuralUnit')
set_module_hyperparameters(key='neuralUnit2', hyperparameters={'weights': [1, 1], 'bias': -1, 'activation': lambda x: x if x > 0 else 0})
...
set_module_hyperparameters(key='arrayMerger', hyperparameters={'index': [0, 1]})
...
set_module_hyperparameters(key='neuralUnit3', hyperparameters={'weights': [1, -2], 'bias': 0, 'activation': lambda x: x if x > 0 else 0})
